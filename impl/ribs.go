...
func (r *ribs) withGroup(prefer iface.GroupKey, cb func(group *Group) error) (selectedGroup iface.GroupKey, err error) {
	r.lk.Lock()
	defer r.lk.Unlock()
	...
	selectedGroup = iface.UndefGroupKey
	{
		...
		if selectedGroup != iface.UndefGroupKey {
			g, err := OpenGroup(r.db, r.index, selectedGroup, r.root, true, false)
			if err != nil {
	...
	g, err := OpenGroup(r.db, r.index, selectedGroup, r.root, true, true)
	if err != nil {
		return iface.UndefGroupKey, xerrors.Errorf("opening group: %w", err)
	}
...
func (r *ribBatch) Put(ctx context.Context, c []mh.Multihash, datas [][]byte) error {
	...
	var done int
	for done < len(c) {
		gk, err := r.r.withGroup(r.currentWriteTarget, func(g *Group) error {
